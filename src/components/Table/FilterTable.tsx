...
export function UuiCompositeTable<T>({ schema, sortInfo, pageInfo, update, data, Actions }: UuiTableProps<T>) {
    ...
    return <Fragment>
        <UuiTableContainer>
            <UuiTable aria-label='simple table'>
                <UuiTableHead>
                    <UuiTableRow className='userManagement-staffMngTable__tablerow Uui-table__row'>
                        {schema.columns.map((column) => (
                            <TableHeaderCell
                                schema={column}
                                sortInfo={{ id: request?.sort ?? schema.columns[0]?.id ?? '', direction: request?.direction }}
                                updateSortSchema={({ id: sort, direction }) => setRequest(s => ({ ...s, sort, direction }))}
                            />
                        ))}
                        {Actions &&
                            <UuiTableCell align='right'>
                                <UuiTypography className='Uui-txt-bold'></UuiTypography>
                            </UuiTableCell>
                        }
                    </UuiTableRow>
                </UuiTableHead>
